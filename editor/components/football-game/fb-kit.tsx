...
import { FB_TEAM_COLOR_PRESETS } from '@smelter-editor/types';
...
/* ------------------------------------------------------------------ *
 * Touchline kit — the design language of the football game
 * (docs/design/touchline/*.dc.html): asphalt plates with one cut corner
 ...
 * 2/3); phones use <FbPhoneShell>. The broadcast HUD (server/src/inputs/
 * FbHud.tsx + scripts/fb-render-assets.mjs) burns the same tokens.
 * ------------------------------------------------------------------ */
...
/** Touchline tokens (apps variant — the HUD uses #F4EFE6 / #33E1FF). */
export const FB = {
  page: '#0B1220',
  asphalt: '#0B1220',
  dark: '#0B1220',
  chalk: '#E8E4DA',
  chalkHud: '#F4F1E8',
  /** The accent (grass) — the key keeps its Blacktop name so the kit stays diffable. */
  electric: '#2FBF71',
  electricHud: '#2FBF71',
  gold: '#E8B33A',
  ...
  bad: '#FF3B3B',
  ballOrange: '#F4F1E8',
  plate: 'rgba(11,18,32,.94)',
  plate2: 'rgba(18,27,44,.94)',
  /** Chip scrim over live video. */
  scrim: 'rgba(11,18,32,.94)',
...
/** Font stacks wired by app/football-game/fonts.ts (next/font vars). */
export const fbDisplay =
  "var(--font-fb-display), 'Big Shoulders Display', 'Big Shoulders', sans-serif";
export const fbMono = "var(--font-fb-mono), 'IBM Plex Mono', monospace";
...
/** How an entry came about, for a ledger row ("AI 92%" / "MANUAL"). */
export function aiGuessLabel(event: FbEventEntry): string {
  if (event.source === 'manual') return 'MANUAL';
  return `AI ${Math.round(event.aiConfidence * 100)}%`;
}

/* ------------------------------- stage ------------------------------- */

/** The 1280×720 arcade stage on asphalt. */
export function FbStage({ children }: { children: React.ReactNode }) {
  return <ArcadeStage background={FB.page}>{children}</ArcadeStage>;
}
    ...
    <span
      ...
      <span
        style={{
          ...
          height: barH,
          background: accent,
          margin: `0 ${Math.round(size * 0.07)}px 0 ${Math.round(size * 0.09)}px`,
        }}
      />
...
/**
 * The Touchline plate: one element, plate fill, one cut corner, no hairline.
 * Team/tone colour lives in a 4–16 px bar drawn inside the clip (so it is
 * cut with the corner, as in the design).
 */
export function FbPlate({
  fill = FB.plate2,
  cutPx = 18,
  leftBar,
  leftBarColor = FB.electric,
  topBar,
  topBarColor = FB.electric,
  bottomBar,
  bottomBarColor = FB.electric,
  border,
  texture,
  className,
  style,
...
export type FbButtonVariant =
  ...
  | 'dangerSolid'
  | 'good';
export type FbButtonSize = 'lg' | 'md' | 'sm';
...
export function FbButton({
  ...
  return (
    <button
      type='button'
      className={`fb-btn${active && !disabled ? ' fb-breathe' : ''}`}
...
export function Stepper({
  ...
  return (
    <div
      ...
      <span
        style={{
          fontFamily: font === 'display' ? fbDisplay : fbMono,
...
export function NameField({
  ...
  autoFocus?: boolean;
  /** Enter in the field (a phone keyboard's GO) submits. */
  onEnter?: () => void;
  style?: React.CSSProperties;
}) {
  return (
    ...
    <div
      ...
      <label
        ...
        style={{
          position: 'relative',
          aspectRatio: '1',
          border: custom ? `3px solid ${FB.chalk}` : `1px dashed ${FB.rule3}`,
          boxSizing: 'border-box',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
  ...
  return (
    <span
      style={{
        ...
        height: h,
        background: color,
        outline: needsOutline(color) ? `1px solid ${FB.rule2}` : undefined,
        flexShrink: 0,
        ...style,
      }}
    />
...
export function ScoreRow({
  ...
  teams: Record<FbTeamId, { name: string; color: string; score: number }>;
  nameSize?: number;
  scoreSize?: number;
  ...
  return (
    <div
      ...
      <Display size={Math.round(scoreSize * 0.8)} weight={700} color={FB.dim2}>
        {separator}
      </Display>
      <Display
        size={scoreSize}
  ...
  const border =
    tone === 'rec'
      ...
      : tone === 'idle'
        ? `1px solid ${FB.rule3}`
        : 'none';
  const dotColor = tone === 'rec' ? FB.bad : fg;
  return (
    <span
      style={{
        display: 'inline-flex',
        ...
        background: tone === 'rec' || tone === 'idle' ? FB.plate : solidBg,
        color: fg,
        border,
        boxSizing: 'border-box',
        fontFamily: fbMono,
      ...
      {dot ? (
        <span
          ...
          style={{
            ...
            border: tone === 'idle' ? `1px solid ${FB.chalk}` : undefined,
            boxSizing: 'border-box',
            flexShrink: 0,
          }}
        />
          ...
          : tone === 'chalk'
            ...
            : tone === 'gold'
              ...
              : tone === 'good'
                ? FB.good
                : 'transparent';
  const fg = tone === 'bad' || tone === 'outline' ? FB.chalk : FB.dark;
  return (
    <span
      style={{
        display: 'inline-block',
        background: bg,
        color: fg,
        border: tone === 'outline' ? `1px solid ${FB.rule2}` : 'none',
  ...
  return (
    <div
      style={{
        ...
        width,
        height,
        background: FB.fillStrong,
        overflow: 'hidden',
        ...style,
      }}>
      <div
        className={indeterminate ? 'fb-sweep-bar' : undefined}
        style={{
          position: 'absolute',
          left: 0,
          top: 0,
...
export function QrBox({
  ...
  return (
    <div
      style={{ background: FB.chalk, padding: pad, lineHeight: 0, ...style }}>
      <QRCode value={url} size={size} fgColor='#141416' bgColor={FB.chalk} />
    </div>
  );
}
...
export function WarnPlate({
  ...
  const color = tone === 'bad' ? FB.bad : FB.amber;
  return (
    <FbPlate
      cutPx={cutPx}
      leftBar={title ? 6 : 4}
...
export function HazardStrip({
  ...
  const color = tone === 'bad' ? FB.bad : tone === 'good' ? FB.good : FB.amber;
  const fg = tone === 'bad' ? FB.chalk : FB.dark;
...
export function ConfirmCard({
  ...
  return (
    <div
      ...
      <Display size={Math.round(22 * scale)} weight={800} color={FB.bad}>
        {title}
      </Display>
      {copy ? (
        <Copy
      ...
      <div
        className='fb-disarm-bar'
        style={{ '--fb-disarm-ms': `${timeoutMs}ms` } as React.CSSProperties}
      />
    </div>
  );
}
...
type TeamsLite = Record<
  FbTeamId,
  { name: string; color: string; short?: string }
>;

/** Match clock of an entry ("12:41"). */
function eventClock(e: FbEventEntry): string {
  const s = Math.max(0, Math.floor(e.clockMs / 1000));
  return `${Math.floor(s / 60)}:${String(s % 60).padStart(2, '0')}`;
}

/**
 * One ledger entry: stripe · KIND · NAME · meta, with the moderator's
 * controls (assign A / B, VOID) when handlers are passed.
 */
export function LedgerRow({
  ...
  const stripeColor = event.team
    ? teams[event.team].color
    : pending
      ? FB.amber
      : FB.rule2;
  const kindSize = Math.round((dense ? 18 : 22) * scale);
  const nm = Math.round((dense ? 18 : 22) * scale);
  const meta = `${showIndex ? `#${event.index} · ` : ''}${eventClock(event)} · ${aiGuessLabel(event)}${event.detail ? ` · ${event.detail}` : ''}${event.period === 2 ? ' · 2H' : ''}${voided ? ' · VOID' : pending ? ' · PENDING' : ''}`;
  return (
    <div
      style={{
        display: 'flex',
      ...
      <Display
        ...
        weight={900}
        color={
          voided ? FB.dim : goal ? FB.electric : pending ? FB.amber : FB.chalk
        }
        style={{
          minWidth: Math.round(70 * scale),
          textDecoration: voided ? 'line-through' : undefined,
        }}>
        {goal && pending ? 'GOAL?' : eventLabel(event.kind)}
      </Display>
      <Display
        size={nm}
        weight={800}
        color={voided ? FB.dim : FB.chalk}
        style={{
          overflow: 'hidden',
          textOverflow: 'ellipsis',
          whiteSpace: 'nowrap',
      ...
      {onAssign || onVoid ? (
        <div
          ...
          {onAssign && (goal || pending)
            ? (['A', 'B'] as const).map((t) => {
                const on = event.team === t && !voided;
                return (
                  <Chip
                    key={t}
                    dense
                    label={teams[t].short ?? teams[t].name}
                    active={on}
...
/**
 * REF CALL card: a goal candidate for the moderator. Team buttons keep the
 * text chalk on a plate with a 2 px team border + stripe, NO GOAL in red outline.
 */
export function RefCallCard({
  ...
  const leadTeam = event.team;
  const teamBtn = (t: FbTeamId) => (
    <button
      key={t}
      type='button'
      className='fb-btn'
      ...
      <div
        ...
        <div
          style={{
            ...
            background:
              'radial-gradient(ellipse at 50% 40%,#1d3a2b,#0b1220 75%)',
            flexShrink: 0,
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
          }}>
          <Display size={Math.round(20 * scale)} weight={900} color={FB.amber}>
            ...
            {event.source !== 'ai' ? (
              ...
              <>
                ...
                <span
                  style={{
                    fontWeight: 600,
                    color: leadTeam ? teams[leadTeam].color : FB.chalk,
                    opacity: 1,
                  }}>
                  {leadTeam ?? '?'} {Math.round(event.aiConfidence * 100)}%
                </span>
                {event.side ? ` · ${event.side.toUpperCase()} GOAL` : ''}
              </>
            )}
          </Mono>
          {event.detail ? (
  ...
  return (
    <button
      ...
      style={{
        ...
        background: FB.plate,
        border: on ? '1px solid rgba(255,46,61,.5)' : `1px solid ${FB.rule2}`,
        color: FB.chalk,
        fontFamily: fbMono,
        fontWeight: 600,
      ...
      <span
        className={on && !rec.isWaitingForDownload ? 'fb-pulse' : undefined}
        style={{
          width: 10,
          height: 10,
          borderRadius: '50%',
          background: on ? FB.bad : 'transparent',
          border: on ? 'none' : `1px solid ${FB.chalk}`,
          boxSizing: 'border-box',
        }}
      />
      {label}
...
export function FbRecordingPlate({ rec }: { rec: KbtRecording }) {
  return (
    <div
      style={{
        display: 'flex',
...
export function FbDevicePickers({ rig }: { rig: CommentatorRig }) {
