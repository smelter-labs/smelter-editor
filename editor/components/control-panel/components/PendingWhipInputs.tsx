...
import { addCameraInput, updateInput, updateRoom, getRoomInfo } from '@/app/actions/actions';
...
export function PendingWhipInputs({
    ...
    const [connectType, setConnectType] = useState<'camera' | 'screenshare' | null>(null);
    ...
    const handleConnect = async (
        ...
        const streamRef = type === 'camera' ? cameraStreamRef : screenshareStreamRef;
        const setActiveInputId = type === 'camera' ? setActiveCameraInputId : setActiveScreenshareInputId;
        const setIsActive = type === 'camera' ? setIsCameraActive : setIsScreenshareActive;
        const publishFn = type === 'camera' ? startPublish : startScreensharePublish;
        ...
        try {
            ...
            toast.success(`Connected ${type === 'camera' ? 'camera' : 'screenshare'}: ${pendingInput.title}`);
        } catch (e: any) {
                ...
                <div
                    ...
                    <div className='flex gap-2'>
                        ...
                        <Button
                            ...
                            {connectingId === pendingInput.id && connectType === 'screenshare' ? (
                                <LoadingSpinner size='sm' variant='spinner' />
                            ) : (
                                <>
