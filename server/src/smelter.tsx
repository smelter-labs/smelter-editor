...
class SmelterManager {
  ...
  public async init() {
    ...
    if (haveDuckArt) {
      ...
      console.warn(
        '[smelter] imgs/ducks missing — duck sprites not registered',
      );
    }
    ...
    if (haveGhostArt || haveDuckArt) {
      ...
      console.warn(
        '[smelter] no ghost or duck art — haunter sprites not registered',
      );
    }
    ...
    // Kettlebell tournament broadcast chrome: pre-rendered design fragments
    // (frames, plates, headers) generated by scripts/kbt-render-assets.mjs.
    // Ids are `kbt-<basename>` — must match KbtHud.tsx.
    const kbtDir = path.join(__dirname, '../imgs/kbt');
    if (await pathExists(kbtDir)) {
      for (const file of await readdir(kbtDir)) {
        if (!file.endsWith('.png')) continue;
        await this.registerImage(`kbt-${file.slice(0, -4)}`, {
          serverPath: path.join(kbtDir, file),
          assetType: 'png',
        });
      }
    } else {
      console.warn(
        '[smelter] imgs/kbt missing — kettlebell HUD art not registered',
      );
    }

    // Basketball game ("Blacktop") broadcast chrome, generated by
    // scripts/bb-render-assets.mjs. Ids are `bb-<basename>` — must match BbHud.tsx.
    const bbDir = path.join(__dirname, '../imgs/bb');
    if (await pathExists(bbDir)) {
      for (const file of await readdir(bbDir)) {
        if (!file.endsWith('.png')) continue;
        await this.registerImage(`bb-${file.slice(0, -4)}`, {
          serverPath: path.join(bbDir, file),
          assetType: 'png',
        });
      }
    } else {
      console.warn(
        '[smelter] imgs/bb missing — basketball HUD art not registered',
      );
    }

    // Football game ("Touchline") broadcast chrome, generated by
    // scripts/fb-render-assets.mjs. Ids are `fb-<basename>` — must match FbHud.tsx.
    const fbDir = path.join(__dirname, '../imgs/fb');
    if (await pathExists(fbDir)) {
      for (const file of await readdir(fbDir)) {
        if (!file.endsWith('.png')) continue;
        await this.registerImage(`fb-${file.slice(0, -4)}`, {
          serverPath: path.join(fbDir, file),
          assetType: 'png',
        });
      }
    } else {
      console.warn(
        '[smelter] imgs/fb missing — football HUD art not registered',
      );
    }
    ...
    const fontFiles = [
      ...
      // Kettlebell tournament broadcast HUD (see KbtHud.tsx): display face +
      // mono labels, weights map to Smelter's fontWeight names
      // (500=medium, 700=bold, 800=extra_bold, 400=normal, 600=semi_bold).
