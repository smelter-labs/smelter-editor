...
export function PrimaryOnLeftLayout() {
  ...
  const secondaryWidth = isVertical
    ? resolution.width
    : resolution.width - primaryWidth;
  const secondaryHeight = isVertical
    ? resolution.height - primaryHeight
    : resolution.height;
  ...
  if (isVertical) {
    tileW = Math.round(
      (secondaryWidth - TILES_PADDING * (prevTileCount + 1)) / prevTileCount,
    );
    ...
    tileH = Math.round(
      (secondaryHeight - TILES_PADDING * (prevTileCount + 1)) / prevTileCount,
    );
    incomingStartTop =
      TILES_PADDING + swap.incomingPrevIndex * (tileH + TILES_PADDING);
    ...
    <View
      ...
      <View
        ...
        <View
          style={
            isVertical
              ? { width: resolution.width, height: primaryHeight }
              : { width: primaryWidth, height: resolution.height }
          }>
          <Rescaler
            style={
              isVertical ? { height: primaryHeight } : { width: primaryWidth }
            }>
              ...
              style={{
                ...
                height: swapOutgoingEnabled
                  ? isVertical
                    ...
                    : resolution.height -
                      swap.progress * (resolution.height - tileH)
                  : isVertical
                    ? primaryHeight
                    : resolution.height,
              }}>
        ...
        <Rescaler
          style={{
            ...
            width:
              tileW +
              swap.progress *
                ((isVertical ? resolution.width : primaryWidth) - tileW),
            height:
              tileH +
              swap.progress *
                ((isVertical ? primaryHeight : resolution.height) - tileH),
          }}>
